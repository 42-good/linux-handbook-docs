...
AMD不同架构的显卡需要不同驱动，但大多数使用`amdgpu`（专有驱动）即可。或者，对于自由驱动，您可以使用`mesa-va-drivers`。
...
NVIDIA显卡在专业用途下可能需要`akmod-nvidia`与`xorg-x11-drv-nvidia-cuda`库。
